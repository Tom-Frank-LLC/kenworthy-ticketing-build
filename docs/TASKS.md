...
**Nothing has ever called Square from this path.** The request shapes for Customers / Orders / Invoices are reasoned from the API, not observed. Production's `SQUARE_ENV` points at the live account, so the first `Generate Invoice` there creates a draft in the real Square account — it sends and charges nothing, but do the first run on **staging** and compare it line for line against a real Kenworthy event invoice (tax, terms, wording) first. The contract restyle is confirmed — Tom looked at the deployed `/contract/:token` on 2026-08-14 and it renders correctly.
