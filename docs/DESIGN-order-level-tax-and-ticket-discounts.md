...
**Status:** decisions taken (§8); **Ship 1 built and verified on staging** (§9); Ship 2 not started.
...
2. **Two ships.**
3. **Partial refund of a quantity-discounted order:** *open.* A full refund returns exactly what
   was paid (10 tickets at $6 = $60 + tax) and needs no change. The open case is refunding 7 of
   those 10, leaving 3 tickets held at a price only offered for 4+. Options: (a) allow, staff
   discretion, with a warning on the refund screen — recommended; (b) block below
   `min_quantity`; (c) claw the discount back out of the refund.
...
**Not yet done for Ship 1:** production. The migration is applied to staging only, and
`ticket-checkout` / `square-cash-sale` are deployed to staging only. `film-pass-checkout` and
`square-donation` changed by one removed field each and have not been redeployed anywhere.
