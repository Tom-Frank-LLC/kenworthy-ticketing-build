-- A film can be ticketed somewhere else. BRIEF-movie-external-ticketing.
--
-- Events and live performances have carried this for a month: ticket_type
-- (ticketed | rsvp | info_only) says how people get in, and rsvp_url is where
-- they go when the answer is "not here". Movies had neither, so a film sold
-- through an outside site — a festival's own box office, a distributor's
-- platform — could only be listed as a ticketed showing that then had to be
-- priced, given Square variations, and quietly not promoted.
--
-- Same enum, same columns, same default. Every existing film is 'ticketed',
-- which is what it has always been.
--
-- Unlike an RSVP event, an externally ticketed film still has showings: the
-- dates are real and belong on the calendar; only the ticket is bought
-- elsewhere. So the showing rows stay, and what changes is that nothing may
-- sell against them here — which is the second half of this file.

ALTER TABLE public.movies
  ADD COLUMN ticket_type public.event_ticket_type NOT NULL DEFAULT 'ticketed',
  ADD COLUMN rsvp_url text;

COMMENT ON COLUMN public.movies.ticket_type IS
  'How people get in. Matches events.ticket_type so one admin control serves every production. rsvp / info_only showings are refused by price_ticket_order.';
COMMENT ON COLUMN public.movies.rsvp_url IS
  'Where tickets are sold when ticket_type = rsvp. Cleared by the form for any other mode.';

-- Movies are read by the public through a column-level grant (20260701020754),
-- not a table-level one, so a new public column is invisible to the site until
-- it is named here. Without this the feed would read `undefined` and render
-- every externally ticketed film as an ordinary one.
GRANT SELECT (ticket_type, rsvp_url) ON public.movies TO anon;

-- ---------------------------------------------------------------------------
-- The boundary. price_ticket_order is the one door every paid ticket row goes
-- through — online checkout and the box office both — and it already refuses a
-- walk-in night, a finished show and a house closed by hand. A production that
-- is not ticketed here joins that list, for all three tables at once: an RSVP
-- event with a showing row (the form does not create them, but nothing stops a
-- row existing) gets the same refusal a film does.
--
-- Comps are deliberately not refused. create_ticket_order's comp branch does
-- not price and does not come through here; a $0 pass for a guest at a film the
-- theatre is hosting but not selling is still a thing the box office may do.
--
-- Body otherwise identical to 20260922161316: the production lookup also reads
-- ticket_type, and one IF follows it.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- The production: title, category, the surcharge opt-in, how people get in.
  IF s.event_id IS NOT NULL THEN
    SELECT title, COALESCE(pass_processing_fee, false), ticket_type::text
      INTO v_title, v_pass_fee, v_ticket_type FROM public.events WHERE id = s.event_id;
    v_title := COALESCE(v_title, 'Kenworthy event'); v_category := 'Special Events';
  ELSIF s.live_performance_id IS NOT NULL THEN
    SELECT title, COALESCE(pass_processing_fee, false), ticket_type::text
      INTO v_title, v_pass_fee, v_ticket_type FROM public.live_performances WHERE id = s.live_performance_id;
    v_title := COALESCE(v_title, 'Kenworthy performance'); v_category := 'Live Performances';
  ELSE
    SELECT title, COALESCE(pass_processing_fee, false), ticket_type::text
      INTO v_title, v_pass_fee, v_ticket_type FROM public.movies WHERE id = s.movie_id;
    v_title := COALESCE(v_title, 'Kenworthy showing'); v_category := 'Films';
  END IF;

  -- Not sold here. The same sentence the showing page shows, so a stale tab or
  -- a direct call is told what the page would have said. NULL (a production
  -- row that could not be read) is not refused on this ground — the sale fails
  -- further down for its own reasons, and this must not misdescribe it.
  IF v_ticket_type IN ('rsvp', 'info_only') THEN
    RAISE EXCEPTION 'Tickets for this showing are not sold here.' USING ERRCODE = 'PT409';
  END IF;
